...
class ScenarioService {
  ...
  List<Body> _generateRandomBodies(AppLocalizations? l10n) {
    ...
    for (int i = 0; i < SimulationConstants.numberOfStars; i++) {
      final angle =
          ...
          (_random.nextDouble() - 0.5) * SimulationConstants.starAngleRandomness; // triangle with some randomness

      final distance =
          SimulationConstants.starDistanceMin +
          _random.nextDouble() * (SimulationConstants.starDistanceMax - SimulationConstants.starDistanceMin);

      final height =
          (_random.nextDouble() - 0.5) * SimulationConstants.starHeightRange; // expanded z height for maximum 3D spread

      final position = vm.Vector3(distance * math.cos(angle), distance * math.sin(angle), height);

      // Calculate velocity for roughly circular motion with guaranteed 3D characteristics
      final orbitalSpeed =
          SimulationConstants.starOrbitalSpeedMin +
          _random.nextDouble() * (SimulationConstants.starOrbitalSpeedMax - SimulationConstants.starOrbitalSpeedMin);
      ...
      final zVelocityMagnitude =
          ...
          _random.nextDouble() * (SimulationConstants.starZVelocityMax - SimulationConstants.starZVelocityMin);
      ...
      final velocity = vm.Vector3(
        orbitalSpeed * math.cos(tangentAngle) +
            (_random.nextDouble() - 0.5) * SimulationConstants.starVelocityRandomness,
      ...
      // Assign star names
      final starNames = l10n != null ? [l10n.bodyAlpha, l10n.bodyBeta, l10n.bodyGamma] : ['Alpha', 'Beta', 'Gamma'];

      // Generate star mass and radius first
      final starMass =
          SimulationConstants.starMassMin +
          _random.nextDouble() * (SimulationConstants.starMassMax - SimulationConstants.starMassMin);

      final starRadius =
          SimulationConstants.starRadiusMin +
          _random.nextDouble() * (SimulationConstants.starRadiusMax - SimulationConstants.starRadiusMin);
      ...
      final avgMass = (SimulationConstants.starMassMin + SimulationConstants.starMassMax) / 2;
      final massRatio = starMass / avgMass;
      final stellarLuminosity = SimulationConstants.solarLuminosity * math.pow(massRatio, 3.5);
    ...
    final planetDistance =
        ...
        _random.nextDouble() * (SimulationConstants.planetDistanceMax - SimulationConstants.planetDistanceMin);

    final planetHeight =
        (_random.nextDouble() - 0.5) * SimulationConstants.planetHeightRange; // expanded z range for enhanced 3D motion
    ...
    final planetOrbitalSpeed =
        ...
        _random.nextDouble() * (SimulationConstants.planetOrbitalSpeedMax - SimulationConstants.planetOrbitalSpeedMin);
    ...
    final planetZVelocityMagnitude =
        ...
        _random.nextDouble() * (SimulationConstants.planetZVelocityMax - SimulationConstants.planetZVelocityMin);
    ...
    final planetVelocity = vm.Vector3(
      planetOrbitalSpeed * math.cos(planetTangentAngle) +
          (_random.nextDouble() - 0.5) * SimulationConstants.planetVelocityRandomness,
    ...
    if (planetMassType < SimulationConstants.smallPlanetProbability) {
      ...
      planetMass =
          ...
          _random.nextDouble() * (SimulationConstants.smallPlanetMassMax - SimulationConstants.smallPlanetMassMin);
      planetRadius =
          SimulationConstants.smallPlanetRadiusMin +
          _random.nextDouble() * (SimulationConstants.smallPlanetRadiusMax - SimulationConstants.smallPlanetRadiusMin);
    } else if (planetMassType < SimulationConstants.earthLikePlanetProbability) {
      // Earth-like planet: 40% chance
      planetMass =
          SimulationConstants.earthLikePlanetMassMin +
          _random.nextDouble() *
              (SimulationConstants.earthLikePlanetMassMax - SimulationConstants.earthLikePlanetMassMin);
      planetRadius =
          SimulationConstants.earthLikePlanetRadiusMin +
          _random.nextDouble() *
              (SimulationConstants.earthLikePlanetRadiusMax - SimulationConstants.earthLikePlanetRadiusMin);
    } else {
      // Super-Earth: 30% chance
      planetMass =
          SimulationConstants.superEarthMassMin +
          _random.nextDouble() * (SimulationConstants.superEarthMassMax - SimulationConstants.superEarthMassMin);
      planetRadius =
          SimulationConstants.superEarthRadiusMin +
          _random.nextDouble() * (SimulationConstants.superEarthRadiusMax - SimulationConstants.superEarthRadiusMin);
    }
    ...
    bodies.add(
      Body(
        ...
        mass: planetMass,
        radius: planetRadius,
        color: colors[3 + _random.nextInt(colors.length - 3)], // random color from remaining palette
        name: planetName,
        isPlanet: true,
  ...
  List<Body> _generateEarthMoonSun(AppLocalizations? l10n) {
    ...
    bodies.add(
      Body(
        ...
        mass: 45.0, // Further reduced from 60.0 to minimize Sun's disruptive influence
    ...
    final earthDistance = 50.0; // Same as solar system Earth distance for consistency
    final earthSpeed = math.sqrt(1.2 * 45.0 / earthDistance) * 1.0; // Updated for new Sun mass
    ...
    final earthBaseVx = earthSpeed * math.sin(earthAngle); // Flipped sign for counterclockwise
    final earthBaseVz = -earthSpeed * math.cos(earthAngle); // Flipped sign for counterclockwise
    ...
    bodies.add(
      Body(
        ...
        mass: 0.30, // Maximally increased from 0.20 to completely dominate Moon's orbit
        radius: 0.6,
    ...
    final moonDistance = 0.35; // Further increased from 0.30 for maximum Hill sphere
    final moonSpeed = math.sqrt(1.2 * 0.30 / moonDistance) * 0.94; // Updated for new Earth mass and distance

    // Moon orbital inclination: reduced for visual stability in three-body system
    final moonInclinationRadians = 1.0 * math.pi / 180.0; // Reduced from 5.14° to 1.0° for stability
    ...
    final moonZ = earthZInclined + moonDistance * math.sin(moonAngle) * math.cos(moonInclinationRadians);
    final moonY = earthY + moonDistance * math.sin(moonAngle) * math.sin(moonInclinationRadians);

    // Moon velocity = Earth's velocity + Moon's orbital velocity around Earth (with inclination)
    // For counterclockwise motion when viewed from above (positive Y)
    final moonOrbitalVelX = moonSpeed * math.sin(moonAngle); // Flipped sign for counterclockwise
    final moonOrbitalVelZ = -moonSpeed * math.cos(moonAngle) * math.cos(moonInclinationRadians); // Flipped sign
    final moonOrbitalVelY = -moonSpeed * math.cos(moonAngle) * math.sin(moonInclinationRadians); // Flipped sign

    bodies.add(
      Body(
        position: vm.Vector3(moonX, moonY, moonZ),
        velocity: vm.Vector3(earthVx + moonOrbitalVelX, earthVy + moonOrbitalVelY, earthVz + moonOrbitalVelZ),
        mass: 0.015, // Significantly increased Moon mass from 0.008 for maximum Earth-Moon coupling
  ...
  List<Body> _generateBinaryStars(AppLocalizations? l10n) {
    ...
    final starMass = 8.0; // Much smaller mass (Earth-Moon-Sun Sun is 45.0 but dominates completely)
    ...
    final binaryOrbitalSpeed = math.sqrt(gravitationalConstant * starMass / starSeparation) * 0.50; // Very conservative
    ...
    bodies.add(
      Body(
        ...
        velocity: vm.Vector3(0, -binaryOrbitalSpeed * starDirectionMultiplier, 0),
        mass: starMass,
        radius: 1.5,
    ...
    final planetDistance = 60.0; // Far enough to be stable (like Earth at 50.0 from Sun)
    final totalStarMass = starMass * 2;
    final planetSpeed =
        math.sqrt(gravitationalConstant * totalStarMass / planetDistance) * 1.0; // Same factor as Earth-Moon-Sun Earth
    ...
    final moonSpeed =
        math.sqrt(gravitationalConstant * 0.25 / moonDistance) * 0.94; // Same factor as Earth-Moon-Sun Moon
    ...
    bodies.add(
      Body(
        position: vm.Vector3(moonX, planetY + moonRelativeY, planetZ + moonRelativeZ),
        velocity: vm.Vector3(planetVx + moonRelativeVx, planetVy + moonRelativeVy, planetVz + moonRelativeVz),
  ...
  List<Body> _generateAsteroidBelt(AppLocalizations? l10n) {
    ...
    final outerCompanionDistance = 24.0; // Slightly further from belt (belt goes to 22) for safety
    final outerCompanionSpeed =
        math.sqrt(1.2 * 20.0 / outerCompanionDistance) * 1.05; // Slightly increased velocity for stable orbit

    final outerAngle = 0.0; // 0 degrees - start at positive X-axis for centered appearance
    ...
    final innerCompanionDistance = 5.0; // Well inside the belt (belt starts at 8)
    final innerCompanionSpeed = math.sqrt(1.2 * 20.0 / innerCompanionDistance) * 0.9; // Faster inner orbit
    final innerAngle = math.pi; // Put inner planet at 180 degrees for better balance
  ...
  List<Body> _generateGalaxyFormation(AppLocalizations? l10n) {
    ...
    bodies.add(
      Body(
        position: vm.Vector3.zero(),
        velocity: vm.Vector3.zero(),
        mass: 300.0, // Increased from 200 to 300 for stronger gravitational binding
        ...
        stellarLuminosity: 0.0, // Black holes don't emit light (except accretion disk)
      ),
    );
    ...
    for (int arm = 0; arm < numArms; arm++) {
      final armAngleOffset = (arm * 2.0 * math.pi) / numArms; // 120° between arms
      ...
      for (int i = 0; i < starsPerArm; i++) {
        ...
        if (arm == 0 && i < 2) {
          ...
          radius = i == 0 ? 20.0 : 25.0; // Increased from 12.0/15.0 for stability
          safeRadius = radius;
          ...
          radius = 50.0 + armPosition * 100.0; // Increased minimum: 50 to 150 units (was 40-120)
          ...
          final angle = armAngleOffset - armPosition * 4.0 * math.pi * spiralTightness;

          // Add some randomness to create natural scatter (reduced for stability)
          final radiusVariation = radius + (_random.nextDouble() - 0.5) * 2.0; // Further reduced from 3.0
        ...
        final heightVariation = (_random.nextDouble() - 0.5) * 1.0; // Reduced from 1.5
        ...
        if (arm == 0 && i < 2) {
          ...
          galaxySpeedFactor = i == 0 ? 0.98 : 0.96; // Even closer to orbital speed for stability
        } else {
          // Regular stars get higher than orbital velocity for strong stability margin
          galaxySpeedFactor = 1.15; // Increased from 1.05 to 1.15 for better stability
        }
        ...
        final vz = (_random.nextDouble() - 0.5) * 0.01; // Even more minimal z-velocity

        // Star properties based on galactic position
        final massVariation = 0.8 + _random.nextDouble() * 1.4; // 0.8 to 2.2 solar masses
        ...
        if (arm == 0 && i < 2) {
          ...
          final baseHue = 0.05 + distanceFromCenter * 0.15; // Blue center to red outer
          final hueVariation = baseHue + (_random.nextDouble() - 0.5) * 0.1;
          color = HSVColor.fromAHSV(1.0, hueVariation * 360, 0.8, 0.95).toColor();
        }
        ...
        bodies.add(
          Body(
            ...
            radius: radiusBody,
            color: color,
            name: l10n?.bodyStar((arm * starsPerArm) + i + 1) ?? 'Star ${(arm * starsPerArm) + i + 1}',
            bodyType: BodyType.star,
            stellarLuminosity: stellarLuminosity,
          ),
        );
  ...
  List<Body> _generateSolarSystem(AppLocalizations? l10n) {
    ...
    final planetData = [
      [19.5, 0.01, 0.23, AppColors.planetMercury, 7.0], // Mercury - 0.38x Earth size
      [36.0, 0.02, 0.57, AppColors.planetVenus, 3.4], // Venus - 0.95x Earth size
      ...
      [260.0, 0.15, 6.72, AppColors.planetJupiter, 1.3], // Jupiter - 11.2x Earth size (gas giant)
      [479.0, 0.10, 5.64, AppColors.planetSaturn, 2.5], // Saturn - 9.4x Earth size (gas giant)
      [960.0, 0.05, 2.40, AppColors.planetUranus, 0.8], // Uranus - 4.0x Earth size (ice giant)
      [1505.0, 0.06, 2.34, AppColors.planetNeptune, 1.8], // Neptune - 3.9x Earth size (ice giant)
    ];
    ...
    for (int i = 0; i < planetData.length; i++) {
      ...
      final sunMass = 50.0;
      final gravitationalConstant = 1.2;
      final orbitalSpeed = math.sqrt(gravitationalConstant * sunMass / distance);
      ...
      // For counterclockwise motion when viewed from above (positive Y)
      final baseVx = orbitalSpeed * math.sin(angle); // Flipped sign for counterclockwise
      final baseVz = -orbitalSpeed * math.cos(angle); // Flipped sign for counterclockwise
